...
if (isset($_GET['logout'])) { 
    ...
    header("Location: menu_manager"); 
    exit; 
}

// --- 3. AUTO-LOGIN VIA COOKIE (REMEMBER ME) ---
if (!isset($_SESSION['owner_logged_in']) && isset($_COOKIE['obsera_remember'])) {
    if (file_exists('auth.json')) {
        $authData = json_decode(file_get_contents('auth.json'), true);
        ...
        if (count($cookieParts) === 2) {
            ...
            if (isset($authData[$cookieUser]) && md5($authData[$cookieUser]['password']) === $cookieHash) {
...
if (isset($_POST['username']) && isset($_POST['password'])) {
    ...
    if (file_exists('auth.json')) {
        ...
        if (isset($authData[$username]) && $authData[$username]['password'] === $password) {
            $_SESSION['owner_logged_in'] = true;
        ...
        $error_msg = "System error: No client accounts configured yet.";
    }
}
...
<body class="p-4 md:p-10 min-h-screen">
    ...
    <script>
        ...
        // Generate the exact URL for this client's mobile menu
        const MOBILE_MENU_URL = `https://obserasolutions.site/mobile?client=${CLIENT_ID}`;
        // Use a free API to generate the QR Code image instantly
    ...
    <div class="max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-6 md:mb-8 border-b border-slate-700 pb-4">
            ...
            <a href="menu_manager.php?logout=true" class="bg-slate-800 hover:bg-red-500/20 text-slate-400 hover:text-red-400 px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-xs md:text-sm transition-all border border-slate-700">Logout</a>
        </div>
        ...
        <div class="flex flex-col md:grid md:grid-cols-3 gap-6 md:gap-8">
            ...
            <div class="w-full md:col-span-1 flex flex-col gap-6 md:sticky md:top-6">
                ...
                <div class="bg-slate-800 border border-slate-700 p-5 md:p-6 rounded-2xl shadow-xl">
                    <h2 class="text-xl font-bold text-sky-400 mb-4">Add New Item</h2>
                    <form id="addItemForm" class="space-y-3 md:space-y-4" onsubmit="handleAddItem(event)">
                        <div>
                            <label class="text-xs font-bold text-slate-400 uppercase">Category</label>
                            <input type="text" id="cat" required placeholder="e.g., Snacks, Biriyani" class="w-full mt-1 bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm">
                        </div>
                        <div>
                            <label class="text-xs font-bold text-slate-400 uppercase">Item Name</label>
                            <input type="text" id="name" required placeholder="e.g., Chicken Roll" class="w-full mt-1 bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm">
                        </div>
                        <div class="hidden">
                        ...
                        <div>
                            ...
                            <textarea id="desc" rows="2" placeholder="Spiced to perfection..." class="w-full mt-1 bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm"></textarea>
                        </div>
                        <div>
                            <label class="text-xs font-bold text-slate-400 uppercase">Upload Image</label>
                            <input type="file" id="imageFile" accept="image/*" required class="w-full mt-1 text-sm text-slate-400 file:mr-2 file:py-2 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-sky-500/10 file:text-sky-400 hover:file:bg-sky-500/20">
                        </div>
    ...
    <script>
// --- BUG-FREE PURE CSS DESIGN INSIDE A 4x6 A4 CUTOUT ---
                    ...
                    <head>
                        ...
                        <style>
                            ...
                            * { 
                            ...
                            .card { 
                                background: linear-gradient(to bottom, #df3135, #f47216); 
                                width: 4in;
                                height: 6in;
                                display: flex;
                            ...
                            .qr-box {
                                ...
                                width: 2.1in;
                                height: 2.1in;
                                box-shadow: 0 10px 20px rgba(0,0,0,0.25), inset 0 0 0 2px #fef08a;
                                display: flex;
                                justify-content: center;
                                align-items: center;
                                margin-bottom: 0.2in;
        ...
        async function loadMenu() {
            ...
            grid.innerHTML = '<p class="text-slate-400 text-sm animate-pulse">Fetching ....</p>';
            ...
            try {
                ...
                items.forEach(item => {
                    html += `
                        <div class="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden flex relative group">
                            ...
                            <button onclick="deleteItem(${item.row})" class="absolute top-2 right-2 bg-red-500/80 md:bg-red-500/20 text-white md:text-red-400 p-1.5 md:p-2 rounded hover:bg-red-500 hover:text-white transition-colors opacity-100 md:opacity-0 md:group-hover:opacity-100 shadow-md md:shadow-none">
                ...
                grid.innerHTML = '<p class="text-red-400 text-sm">Failed to load menu.</p>';
            }
        }
        ...
        async function handleAddItem(e) {
            ...
            try {
                ...
                imgData.append('image', document.getElementById('imageFile').files[0]);
                ...
                const finalImageUrl = imgJson.data.url;

                btn.innerText = "Saving to Menu...";
                
                const sheetData = new URLSearchParams();
                sheetData.append('action', 'add');
                ...
                setTimeout(() => {
                    btn.innerText = "Add to Menu"; btn.classList.replace('bg-emerald-500', 'bg-sky-500'); btn.disabled = false;
                }, 2000);
                setTimeout(loadMenu, 1000);
            } catch (error) {
                alert("Something went wrong!"); btn.innerText = "Add to Menu"; btn.disabled = false;
            }
        }
